<script lang="ts">
  ...
  let sortedRecords = $derived(
    [...filteredRecords].sort((a, b) => {
      ...
      } else if (sortColumn === "tinggi" || sortColumn === "dbh") {
        valA = Number(valA) || 0;
        valB = Number(valB) || 0;
      } else {
  ...
  let paginatedRecords = $derived(
    sortedRecords.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize),
  );
          ...
          >
            <tr class="bg-muted/30 text-[10px] uppercase tracking-widest text-muted-foreground font-black">
              <th class="p-5 pl-8 rounded-tl-md">
                <button class="flex items-center gap-2 hover:text-foreground transition-colors group" onclick={() => handleSort("jenis_tanaman")}>
                  Tanaman
                  {#if sortColumn !== "jenis_tanaman"}
              ...
              <th class="p-5">
                <button class="flex items-center gap-2 hover:text-foreground transition-colors group" onclick={() => handleSort("tinggi")}>
                  Pengukuran
                  {#if sortColumn !== "tinggi"}
                    <ArrowUpDown size={12} class="opacity-0 group-hover:opacity-50 transition-opacity" />
                  {:else if sortDirection === "asc"}
                    <ArrowUp size={12} class="text-emerald-500" />
                  {:else}
              ...
              <tr class="border-b border-border/50 hover:bg-muted/20 transition-colors group {i === paginatedRecords.length - 1 ? 'border-0' : ''}">
                <td class="p-5 pl-8">
                  <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-muted overflow-hidden flex-shrink-0">
                      {#if r.foto_tanaman}
                        <img src={r.foto_tanaman} alt="" class="w-full h-full object-cover" />
                      {:else}
                    ...
                    <div>
                      <div class="font-bold text-foreground flex items-center gap-2 uppercase tracking-tight">
                        {r.jenis_tanaman || "Tanpa Nama"}
                        <span class="px-2 py-0.5 {getConditionColor(r.kondisi_pertumbuhan || '')} text-white text-[8px] font-black uppercase tracking-widest rounded-full">
                          {r.kondisi_pertumbuhan}
                        </span>
                      </div>
                      <div class="text-[10px] text-muted-foreground italic mt-0.5">{r.nama_ilmiah || "-"}</div>
                      <div class="text-[10px] text-emerald-600/80 font-bold uppercase tracking-wider mt-1.5 flex items-center gap-1">
                        <Leaf size={10} /> {r.kode_tanaman}
                      </div>
                    </div>
                  </div>
                </td>
                <td class="p-5">
                  <div class="space-y-1.5">
                    ...
                    {#if r.kategori_tanaman}
                      <div class="inline-block px-2 py-0.5 bg-emerald-500/10 text-emerald-600 rounded-md text-[8px] font-black uppercase tracking-widest mt-1">
                        {r.kategori_tanaman}
                      </div>
                    {/if}
                  </div>
                ...
                <td class="p-5">
                  <div class="space-y-1.5">
                    <div class="flex items-center gap-2 text-xs">
                      <Ruler size={14} class="text-blue-500" />
                      <span class="font-medium text-muted-foreground">Tinggi:</span> <span class="font-bold">{r.tinggi ? `${r.tinggi} cm` : "-"}</span>
                    </div>
                    <div class="flex items-center gap-2 text-xs">
                      <Trees size={14} class="text-indigo-500" />
                      <span class="font-medium text-muted-foreground">DBH:</span> <span class="font-bold">{r.dbh ? `${r.dbh} cm` : "-"}</span>
                    </div>
                    <div class="flex items-center gap-2 text-[10px] font-bold text-muted-foreground mt-1 bg-muted/40 w-fit px-2 py-1 rounded-lg">
                      <Activity size={10} /> Periode: {r.periode_monitoring}
                    </div>
                  </div>
                </td>
                <td class="p-5">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <div class="relative group/btn">
                      <button onclick={() => openViewDrawer(r)} class="p-2 flex items-center justify-center bg-white border border-border hover:border-emerald-500/50 hover:text-emerald-600 rounded-xl transition-all shadow-sm">
                        <Eye size={16} />
                      </button>
                      <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-1.5 bg-foreground text-background text-[10px] font-black uppercase tracking-widest rounded-lg opacity-0 pointer-events-none group-hover/btn:opacity-100 group-hover/btn:-translate-y-1 translate-y-1 transition-all duration-200 whitespace-nowrap z-50 shadow-xl">
                        Rincian
                    ...
                    <div class="relative group/btn">
                      <button onclick={() => initiateDelete(r)} class="p-2 flex items-center justify-center bg-white border border-border hover:border-red-500/50 text-red-500/70 hover:text-red-600 rounded-xl transition-all shadow-sm">
                        <Trash2 size={16} />
                      </button>
                      <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-1.5 bg-foreground text-background text-[10px] font-black uppercase tracking-widest rounded-lg opacity-0 pointer-events-none group-hover/btn:opacity-100 group-hover/btn:-translate-y-1 translate-y-1 transition-all duration-200 whitespace-nowrap z-50 shadow-xl">
                        Hapus
                    ...
                    <div class="relative group/btn">
                      {#if r.periode_monitoring !== "P3"}
                        <button onclick={() => startNextMonitoring(r)} class="p-2 flex items-center justify-center bg-emerald-50 text-emerald-600 border border-emerald-500/20 hover:bg-emerald-600 hover:text-white rounded-xl transition-all shadow-sm">
                          <Plus size={16} />
                        </button>
                        <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-1.5 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg opacity-0 pointer-events-none group-hover/btn:opacity-100 group-hover/btn:-translate-y-1 translate-y-1 transition-all duration-200 whitespace-nowrap z-50 shadow-xl">
                          Monitoring Selanjutnya
                          <div class="absolute top-full left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-emerald-600 rotate-45"></div>
                        </div>
                      {:else}
                        <button disabled class="p-2 flex items-center justify-center bg-muted/50 text-muted-foreground border border-transparent rounded-xl transition-all shadow-sm opacity-50">
